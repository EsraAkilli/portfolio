export const experience = {
  tomBank: {
    position: "Yönetim Stajyeri - Generative AI Takımı",
    company: "TOM Bank",
    duration: "Ağu 2024 - Günümüz",
    location: "İstanbul, Türkiye",
    achievements: [
      "Jira story point'leri için ML destekli tahmin sistemi tasarlıyorum, çoklu skorlama teknikleriyle değerlendiriyorum, event/log streaming ve structlog ile yapılandırılmış loglama uyguluyorum",
      "LLM'ler kullanarak geliştirme görevlerini semantik anlama ve bağlam farkındalıklı görev üretimi için zenginleştiriyorum",
      "Transformers Architecture, Spring AI ve LLM entegrasyon stratejileri hakkında dahili teknik sunumlar veriyorum",
      "Şirketin resmi Medium hesabında AI/ML odaklı makaleler yayınladım, karmaşık kavramları hem teknik hem de iş okuyucuları için erişilebilir içeriğe çevirdim",
      "Prompt engineering yöntemlerini uygulayarak LLM odaklı iş akışlarını optimize ettim",
      "AI uygulamalarında yazılım tasarım prensiplerini analiz eden demo proje oluşturdum, temiz mimari ve sürdürülebilirliğe odaklanarak",
      "Spring Boot, RESTful API tasarımı ve temiz kod standartlarını sağlayarak dijital bankacılık sistemlerinin modernizasyonuna katkıda bulundum",
      "Rancher & Kibana kullanarak mikroservisleri izledim, SonarQube ve Postman koleksiyonları ile test kapsamını iyileştirdim",
      "Jenkins aracılığıyla CI/CD iş akışlarını denetledim, model dağıtımını ve sürüm yönetimini kolaylaştırdım"
    ],
    technologies: ["Python", "SQL", "LLMs", "Hugging Face", "Java", "RESTful APIs", "Jenkins", "Postman"]
  },
  
  macellan: {
    position: "Backend Developer Stajyeri",
    company: "MACELLAN",
    duration: "Ağu 2023 - Ağu 2024",
    location: "İstanbul, Türkiye",
    achievements: [
      "Yüksek kod kalitesi ve test kapsamı elde ederek sağlam Laravel API'leri geliştirdim ve test ettim",
      "Finansal tahmin projesi oluşturdum ve Vue.js ile dinamik kullanıcı arayüzlerine katkıda bulundum",
      "Agile sprint'lere, ürün demo'larına ve paydaş incelemelerine katıldım"
    ],
    technologies: ["Laravel", "PHP", "Vue.js", "REST APIs", "Agile"]
  },
  
  tusas: {
    position: "Mühendis Stajyeri - Üretim Planlama ve Kontrol",
    company: "TUSAŞ-Türk Havacılık ve Uzay Sanayii (TAI)",
    duration: "Tem 2022 - Ağu 2022",
    location: "Ankara, Türkiye",
    achievements: [
      "Havacılık üretim planlama ve kontrol sistemlerinde uygulamalı deneyim kazandım",
      "Yüksek teknolojili havacılık ortamında endüstri mühendisliği metodolojileri ile çalıştım"
    ],
    technologies: ["Üretim Planlama", "Kontrol Sistemleri", "Endüstri Mühendisliği"]
  }
};
